...
In natural units and Heaviside--Lorentz form, Maxwell's equations in the presence of charge and current densities $\varrho(\boldsymbol{r},t)$ and $\boldsymbol{j}(\boldsymbol{r},t)$ read \cite{guidry1991gaugefieldtheories}
\begin{align}
...
\begin{equation}
	\boldsymbol{P}_{\mathrm{NL}}(\boldsymbol{r},t) \supset \boldsymbol{P}_{\mathrm{S}}^{(3)}(t)\,e^{\mathrm{i}(\boldsymbol{k}_\text{S}\cdot\boldsymbol{r}-\omega_{\mathrm{L}} t)} + \text{c.c.}
...
\begin{equation}
	\boldsymbol{E}_{\mathrm{S}}(\boldsymbol{r},t) = \tfrac{1}{2}\,\boldsymbol{E}_{\mathrm{S}}(z,t)\,e^{\mathrm{i}(k'_{\mathrm{S}} z-\omega_{\mathrm{L}} t)} + \text{c.c.},
	\label{eq:app_field_ansatz}
\end{equation}
with dispersion relation $k'_{\mathrm{S}}=n(\omega_{\mathrm{L}})\omega_{\mathrm{L}}$.
...
\begin{equation}
	\left|\partial_t \boldsymbol{E}_{\mathrm{S}}\right| \ll \omega_{\mathrm{L}}\,\left|\boldsymbol{E}_{\mathrm{S}}\right|,
	\qquad
...
\begin{equation}
	\partial_t^2\!\left[\boldsymbol{E}_{\mathrm{S}}(z,t)e^{-\mathrm{i}\omega_{\mathrm{L}} t}\right] \approx -\omega_{\mathrm{L}}^2\,\boldsymbol{E}_{\mathrm{S}}(z,t)e^{-\mathrm{i}\omega_{\mathrm{L}} t},
	\qquad
	\partial_t^2\!\left[\boldsymbol{P}_{\mathrm{S}}^{(3)}(t)e^{-\mathrm{i}\omega_{\mathrm{L}} t}\right] \approx -\omega_{\mathrm{L}}^2\,\boldsymbol{P}_{\mathrm{S}}^{(3)}(t)e^{-\mathrm{i}\omega_{\mathrm{L}} t}.
	\label{eq:app_time_derivatives_svea}
\end{equation}
The carrier oscillation at $\omega_{\mathrm{L}}$ dominates, so second derivatives of the envelopes can be dropped.

\subsection{Envelope Propagation Equation}
\label{app:subsec:envelope_propagation}

Insert \autoref{eq:app_pol_component} and \autoref{eq:app_field_ansatz} into \autoref{eq:app_wave_eq_nonlinear}. Using the SVEA derivative forms from \autoref{eq:app_second_derivative_svea} and \autoref{eq:app_time_derivatives_svea}, and imposing the dispersion relation $k_{\mathrm{S}}'^2=n^2(\omega_{\mathrm{L}})\omega_{\mathrm{L}}^2$, the leading carrier terms cancel. The result is a first-order propagation equation for the envelope,
\begin{equation}
	\mathrm{i} k'_{\mathrm{S}}\,\frac{\partial \boldsymbol{E}_{\mathrm{S}}(z,t)}{\partial z}
	= -\omega_{\mathrm{L}}^2\,\boldsymbol{P}_{\mathrm{S}}^{(3)}(t)\,e^{\mathrm{i}\Delta k\,z},
	\label{eq:app_envelope_propagation}
\end{equation}
where $\Delta k = k_{\mathrm{S}} - k'_{\mathrm{S}}$ is the phase mismatch between the polarisation wavevector and the propagation constant $k'_{\mathrm{S}} = n(\omega_{\mathrm{L}})\omega_{\mathrm{L}}$. This equation shows how the field builds up through the sample, driven locally by the induced polarisation.
...
\begin{equation}
	...
	= \mathrm{i}\,\frac{\omega_{\mathrm{L}}}{n(\omega_{\mathrm{L}})}\,\boldsymbol{P}_{\mathrm{S}}^{(3)}(t)\,L\,e^{\mathrm{i}\Delta k L/2}\,\mathrm{sinc}\!\left(\frac{\Delta k L}{2}\right).
	\label{eq:app_sinc}
\end{equation}
...
\begin{equation*}
	...
	\propto \mathrm{i}\,\frac{\omega_{\mathrm{L}}}{n(\omega_{\mathrm{L}})}\,\boldsymbol{P}_{\mathrm{S}}^{(3)}(t)\,L.
\end{equation*}

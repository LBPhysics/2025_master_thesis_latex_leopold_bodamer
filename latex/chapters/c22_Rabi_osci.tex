...
\begin{equation}
	H_0 = \frac{\omega_0}{2} \sigma_z, \qquad \sigma_z = \ket{e}\bra{e} - \ket{g}\bra{g}
	\label{eq:AtomicHamiltonian}
\end{equation}


\subsubsection{Electric Field}
The electric field is given analogously to \autoref{eq:electric_field_three_pulses} by:
\begin{equation}
	\mathcal{E}(t) = E_0 e^{i\phi} e^{-i\omega_{L} t}, \qquad
	E_{\mathrm{phys}}(t) = \mathcal{E}(t) + \mathcal{E}^*(t)
	\label{eq:ElectricFieldComplex}
\end{equation}
where \(E_0\) is the field amplitude, \(\phi\) is the phase, and \(\omega_{L}\) is the laser frequency.
...
\begin{equation}
	H_{\mathrm{int}}(t) = -\mu \left( \mathcal{E}(t) \sigma_+ + \mathcal{E}^*(t) \sigma_- \right)
	\label{eq:InteractionHamiltonianDipole}
\end{equation}
where \(\mu\) is the dipole matrix element.
\begin{equation}
	V(t) = -\hat{\mu} \left( E(t) \sigma_+ + E^*(t) \sigma_- \right)
	\label{eq:asdasdasd}
\end{equation}
where \(\mu\) is the dipole matrix element.
...
\end{equation}
The time evolution of the system is governed by the time-dependent Schrödinger equation \autoref{eq:oqs_schroedinger_equation}

\subsection{Transform to the Rotating Frame}

Define the unitary transformation:
\begin{equation}
	U(t) = \exp\left(i \omega_{L} t \frac{\sigma_z}{2}\right)
...
\begin{itemize}
	\item \textbf{Free Hamiltonian:}
	      \begin{equation}
		      \tilde{H}_0 = \frac{(\omega_0 - \omega_{L})}{2} \sigma_z = \frac{\Delta}{2} \sigma_z
		      \label{eq:TransformedFreeHamiltonian}
	      \end{equation}
	      where \(\Delta = \omega_0 - \omega_{L}\) is the detuning.

	\item \textbf{Transformed interaction Hamiltonian:}
	      \begin{equation}
		      \tilde{H}_{\mathrm{int}} = -\mu \left( E_0 e^{i\phi} \sigma_+ + E_0 e^{-i\phi} \sigma_- \right)
		      \label{eq:TransformedInteractionHamiltonian}
	      \end{equation}

	\item Defining the Rabi frequency \(\Omega = \mu E_0\), the total rotating-frame Hamiltonian becomes:
	      \begin{equation}
		      \boxed{
			      ilde{H} = \frac{\Delta}{2} \sigma_z - \Omega \left( e^{i\phi} \sigma_+ + e^{-i\phi} \sigma_- \right)
		      }
...
\begin{equation}
	\boxed{
		\begin{aligned}
			\frac{\mathrm{d}}{\mathrm{d}t}\rho_{ee} & = i \Omega_c \rho_{ge} - i \Omega_c^* \rho_{eg} - \gamma_0 \rho_{ee}            \\[6pt]
			\frac{\mathrm{d}}{\mathrm{d}t}\rho_{gg} & = -i \Omega_c \rho_{ge} + i \Omega_c^* \rho_{eg} + \gamma_0 \rho_{ee}           \\[6pt]
			\frac{\mathrm{d}}{\mathrm{d}t}\rho_{eg} & = i \Delta \rho_{eg} + i \Omega_c (\rho_{gg} - \rho_{ee}) - \Gamma \rho_{eg}    \\[6pt]
			\frac{\mathrm{d}}{\mathrm{d}t}\rho_{ge} & = -i \Delta \rho_{ge} - i \Omega_c^* (\rho_{gg} - \rho_{ee}) - \Gamma \rho_{ge}
		\end{aligned}
	}
...
To recover the entries of the original density matrix \(\rho(t)\) from the evolved density matrix in the rotating frame \(\tilde{\rho}(t)\), the inverse transformation is used:
\begin{equation}
...
\begin{align}
	...
	\rho_{ge}(t) & = e^{i\omega_{L} t} \tilde{\rho}_{ge}(t) \label{eq:RecoveryGE}  \\
	\rho_{eg}(t) & = e^{-i\omega_{L} t} \tilde{\rho}_{eg}(t) \label{eq:RecoveryEG}
\end{align}

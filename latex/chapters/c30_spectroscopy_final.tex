...
\label{final:chapter_spectroscopy}

\section{Spectroscopy and Historical Motivation}
\label{final:sec:history}

\noindent
Spectroscopy, in its broadest definition, is the study of the interaction between matter and energy, most often in form of electromagnetic radiation as a function of wavelength or frequency \cite{berman2011principleslaserspectroscopy, mukamel1995principlesnonlinearoptical}. Modern spectroscopic techniques also include the interaction with particles like electrons or ions or acoustic waves \cite{skoogetal2017principlesinstrumentalanalysis}.
The resulting \emph{spectrum} describes the intensity of absorbed, emitted, or scattered light across different frequencies. Because each allowed optical transition corresponds to a specific energy-level difference, spectral features provide direct access to the structure and dynamical properties of a system. An example in medical physics is a finger-clip sensor used in pulse oximetry: by transmitting light through the fingertip at two wavelengths and analysing the absorption difference between oxygenated and deoxygenated haemoglobin, the absorption spectrum of arterial blood reveals the oxygen-saturation level in a non-invasive way~\cite{nitzanetal2014pulseoximetryfundamentals}.
In chemical analysis, absorption spectroscopy works on the simple principle that each dissolved substance absorbs light more strongly at certain wavelengths than others; by shining light through a solution and measuring how much intensity is lost at these characteristic wavelengths, the concentration of specific components---such as alcohols, sugars, or trace metals---can be determined using the Lambert--Beer law~\cite {skoogetal2017principlesinstrumentalanalysis}.

\paragraph{Discrete energy states and the Planck relation.}
Atoms, ions, and molecules occupy discrete, well-separated energy eigenstates. This quantization, introduced historically by Planck in the context of thermal radiation, implies that optical transitions occur only between specific levels and must absorb or emit quanta that match the level spacing \cite{skoogetal2017principlesinstrumentalanalysis}. The associated radiation frequency satisfies
\begin{equation}
    \Delta E = E_1 - E_0 = h\nu = \frac{hc}{\lambda},
    \label{final:eq:planck_relation}
\end{equation}
with Planck's constant $h$, frequency $\nu$, and wavelength $\lambda$. Electronic states dominate for atoms and ions; molecules additionally exhibit vibrational and rotational ladders, leading to richer spectra comprised of lines and bands. At ambient conditions most populations reside in the ground state; spectroscopy perturbs this equilibrium and infers structure and dynamics from absorption, scattering, or emission during relaxation \cite{skoogetal2017principlesinstrumentalanalysis}.


The origins of spectroscopy lie in the observation of such discrete spectral lines in atomic emission and absorption. Bohr’s model (1913) provided the first quantitative explanation by linking these lines to transitions between quantized energy levels, $\Delta E = h\nu$, thereby capturing the characteristic patterns of the Balmer and Rydberg series~\cite{bohr1913constitutionatomsmolecules, rydberg1890rybergsseriesrydberg1890xxxivstructurelinespectra, balmer1885notizuberspectrallinien}. Although formally limited to hydrogen-like systems, this framework introduced the fundamental idea that atomic structure can be inferred directly from the spectrum. Each element exhibits a unique spectral fingerprint%todo ref and use a picture as a explenation~\cite{moore1949atomicenergylevels}.
...
\label{final:sec:concept_light_matter}
\noindent
...
\paragraph{Linear vs. nonlinear response.}
Propagation of an electromagnetic wave through a dielectric induces a polarization of the valence electrons. At modest intensities this polarization is linearly proportional to the field, $P\propto E$, defining the domain of linear spectroscopy. At the high peak fields of lasers, the linear relation breaks down and one expands $P$ as a power series in $E$, consistent with the response expansion used below:
\begin{equation}
    P = \sum_{i\ge 1} c_i E^{i}, \qquad c_i > c_{i+1},
    \label{final:eq:P_power_series}
\end{equation}
where higher-order terms become observable as intensity increases and enable nonlinear spectroscopy.

\paragraph{State manifolds and initial conditions.}
Electronic eigenstates of $H_0$ form the primary ladder in atoms and ions. In molecules, vibrational and rotational structure overlays electronic manifolds, yielding dense level spacings and selection-rule-governed transitions \cite{skoogetal2017principlesinstrumentalanalysis}. Under room-temperature conditions, populations are typically concentrated in the ground state; spectroscopy transiently excites a fraction of the ensemble via heat, electric discharge, optical pulses, particle impact, or chemical reaction. Subsequent absorption, scattering, or emission encodes the return dynamics and level architecture \cite{skoogetal2017principlesinstrumentalanalysis}.

\section{Linear Spectroscopy: Response Functions and Lineshapes}
\label{final:sec:linear}
\noindent
...
\paragraph{Continuum, lines, and bands.}
Experimental spectra often exhibit a smooth continuum background and superimposed sharp \emph{lines} or broader \emph{bands}. Isolated particles acting independently (e.g., low-pressure atomic or ionic gases) yield discrete, narrow lines reflecting well-separated electronic transitions. In molecules, the superposition of vibrational and rotational progressions on electronic transitions produces closely spaced features that merge into band structures, whose envelopes and fine structure are governed by selection rules, temperature, and homogeneous/inhomogeneous broadening \cite{skoogetal2017principlesinstrumentalanalysis}.
...
\begin{equation}
    ...
    \label{final:eq:linear_lineshape}
\end{equation}
...
Even atomic transitions do not produce infinitely sharp lines: finite state lifetimes imply finite energy uncertainty. In time--frequency form one writes a lower bound $\Delta \nu\,\Delta t \gtrsim 1$, giving a \emph{natural linewidth} related to the lifetime of the upper state. Additional mechanisms broaden spectral features, notably Doppler broadening from thermal motion, and collisional (pressure) broadening in dense media, which contribute to inhomogeneous and homogeneous widths depending on the mechanism and timescale.

\section{Extension to Nonlinear Spectroscopy: Third-Order Response and 2D Signals}
\label{final:sec:third_order}
\noindent
Nonlinear spectroscopy leverages coherent, ultrashort laser pulses (treated classically) interacting with matter (treated quantum mechanically) to resolve dynamics on ultrafast timescales (down to tens of femtoseconds) in ensembles. In pump--probe measurements, a pump pulse excites the system and a time-delayed probe monitors its evolution, following simultaneously the system dynamics and the probe field evolution. More generally, the $n$th-order signals correspond to distinct perturbative orders and map directly to physical pathways in the response formalism.
...
\begin{align}
    ...
    \varepsilon_j(t-t_3)\,\varepsilon_k(t-t_3-t_2)\,\varepsilon_\ell(t-t_3-t_2-t_1), \label{final:eq:P3_conv}                            \\
    R^{(3)}_{i j k \ell}(t_3,t_2,t_1) & =
    \left(-\frac{i}{\hbar}\right)^3
    \big\langle
    \big[\!\big[\!\big[\hat{\mu}_i(t_1{+}t_2{+}t_3),\hat{\mu}_j(t_2{+}t_3)\big],\hat{\mu}_k(t_3)\big],\hat{\mu}_\ell(0)\big]
    \big\rangle. \label{final:eq:R3_def}
\end{align}
...
\begin{align}
    ...
    e^{+i\omega_{\mathrm{coh}} t_{\mathrm{coh}}}\,e^{-i\omega_{\mathrm{det}} t_{\mathrm{det}}}\;\varepsilon_{R}(t_{\mathrm{coh}},T,t_{\mathrm{det}}), \label{final:eq:SR} \\
    S_{NR}(\omega_{\mathrm{coh}},T,\omega_{\mathrm{det}})
     & =\int \mathrm{d}t_{\mathrm{coh}}\,\mathrm{d}t_{\mathrm{det}}\;
    e^{-i\omega_{\mathrm{coh}} t_{\mathrm{coh}}}\,e^{-i\omega_{\mathrm{det}} t_{\mathrm{det}}}\;\varepsilon_{NR}(t_{\mathrm{coh}},T,t_{\mathrm{det}}), \label{final:eq:SNR}
\end{align}
...
\label{final:sec:phase_matching}
\noindent
...
\label{final:sec:interpretation}
\noindent

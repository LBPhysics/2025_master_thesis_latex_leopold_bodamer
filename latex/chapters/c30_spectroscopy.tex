...
\begin{equation} \label{eq:electric_field_three_pulses_implemented}\vec{\varepsilon}(t)=\sum_{j=1}^3 \vec{E_0}^j \cdot g_j(t - t_j) \cdot e^{-i (\vec{k}_j \cdot \vec{r} - \omega_j t)} + \text{c.c.},
\end{equation}

\noindent where the normalized Gaussian envelope is defined as

\begin{equation} \label{eq:gaussian_envelope}
...
\begin{align} \label{eq:electric_field_factored}
...
\begin{equation} \label{eq:slow_envelope_with_time_delay}
...
\begin{align}
	\vec{k}_{\mathrm{R}}  & = -\vec{k}_1 + \vec{k}_2 + \vec{k}_3,
	                      & (l, m, n)                         & = (-1,+1,+1), \label{eq:rephasing_selection}    \\
	\vec{k}_{\mathrm{NR}} & = +\vec{k}_1 - \vec{k}_2 + \vec{k}_3,
	                      & (l, m, n)                         & = (+1,-1,+1), \label{eq:nonrephasing_selection}
\end{align}

...
\label{og:subsec:basic_principles}
...
\noindent
Historically, spectroscopic measurements provided the first experimental evidence for the quantization of atomic energy levels, as evidenced by the discrete spectral lines observed in atomic spectra \todoref{bohr1913quantentheorie}.
...
\begin{equation}
	...
	\label{og:eq:wavelength_frequency_relation}
\end{equation}
...
\label{og:subsec:classification_techniques}
...
%% NEW TODO implement into other file
Electromagnetic radiation manifests itself as heat, $\gamma$-rays, X-rays, UV, visible, infrared, microwave, and radio waves. Their photon energies range from $\sim 10^{-6}\,\mathrm{eV}$ (radio waves) to $\sim 10^{6}\,\mathrm{eV}$ ($\gamma$-rays) \cite{griffiths2013introductionelectrodynamics}.
Most properties of such radiation can be described in a classical picture. Plane wave solutions of Maxwell equations contain characteristic quantities like the wavelength $\lambda$, frequency $\nu$, wavevector $\vec{k}$, electric field amplitude $\vec{\varepsilon}$ and the velocity of the wave $\vec{v}$ \cite{griffiths2013introductionelectrodynamics}.
However this description fails to account for absorption and emission processes of radiant energy by matter. Here the quantum nature of light as photons with discrete energies $E = h \nu$ has to be taken into account \cite{cohen-tannoudjietal2008atomphotoninteractionsbasic}.
In light matter interaction, only the electrical component of the transversal electromagnetic wave couples significantly to the charged particles in atoms and molecules. The magnetic component is typically relevant in the interaction with radio-frequencie waves in NMR experiments \cite{skoogetal2017principlesinstrumentalanalysis}.


The phase angle will be important later on, so here it is defined as the offset of the oscillation of a wave with respect to a reference point in time and space. For a wave described as $\vec{\varepsilon}(t) = \vec{\varepsilon}_0 \cos(\vec{k} \cdot \vec{r} - \omega t + \phi)$, the phase $\phi$ determines the initial position of the wave at the origin $t=0$ and $\vec{r}=0$.

If two or more waves overlap in space and time, their relative phases determine the resulting interference pattern. If e.g. two waves have different frequencies $\nu_2 \neq \nu_1$ the superposition of them has a new periodicity of $p = 1 / \Delta \nu$ with $ \Delta \nu = \nu_2 - \nu_1$. This behavior is termed a beat.
\todoidea{include a picture}
A very important observation was done by Jean Baptiste Joseph Fourier (1768 -- 1830). He showed that every periodic function $f(x)$ can be described by a sum of $\sin$ and $\cos$ waves with frequencies $\omega_n$
\begin{equation}
	f(x) = a_0 + \sum_{n=1}^{\infty} \bigl[a_n \cos(\omega_n x) + b_n \sin(\omega_n x)\bigr].
\end{equation}


\noindent
This technique allows one to extract the strength of the individual frequency components that make up a complex time-domain signal. In frequency space, the intensity associated with each frequency directly reveals how strongly that component contributes to the overall signal.

\noindent
Different scattering regimes can be distinguished. Imagine a laser beam incident on a medium containing particles of radius $r$. When the particles are much smaller than the wavelength of the light, $r \ll \lambda$, only a small fraction of the light is scattered out of the beam, and the angular distribution of the scattered light is described by Rayleigh scattering. As the particle size becomes comparable to or larger than the wavelength ($r \sim \lambda$ or $r \gg \lambda$), the scattering pattern becomes more complicated (Mie and geometric scattering), and a larger fraction of the incident intensity is redistributed into different directions.


\noindent
An atomic source of light (for example, a vapour cell) generates unpolarised light because the observed beam is an average over many independent emission events. Each atom emits a photon with a well-defined polarisation relative to its local quantisation axis, but in an ensemble with random orientations the polarisations are averaged out, resulting in an effectively unpolarised beam.

\todo{rewrite:}
The advent of femtosecond laser pulses has made it possible to apply similar concepts in the optical regime. Despite the analogy, the resulting information is different and complementary. In 2D NMR, radio-frequency pulses act on nuclear spins, the relevant timescales are milliseconds or longer, and spin--spin correlation functions are probed. In contrast, 2D electronic spectroscopy uses sequences of ultrashort optical pulses to interrogate electronic transitions on femtosecond to picosecond timescales, providing access to electronic and vibronic coherence dynamics. Similarly, 2D electron spin resonance applies microwave pulses to electronic spins~\cite{zhangetal1999multidimensionalfemtosecondcorrelation}.
...
\label{og:subsec:macroscopic_samples}
...
\begin{equation}
	...
	\label{og:eq:macroscopic_polarization}
\end{equation}
...
\begin{equation}
	\nabla \times \nabla \times \vec{\varepsilon}(\vec{r}, t) + \frac{1}{c^2} \frac{\partial^2}{\partial t^2} \vec{\varepsilon}(\vec{r}, t) = - \frac{4 \pi}{c^2} \frac{\partial^2}{\partial t^2} \vec{P}(\vec{r}, t).
\end{equation}

\noindent
A formal solution of the wave equation shows that the emitted polarization at position $\vec{r}$ and time $t$ is given by

\begin{equation} \label{og:eq:solution_wave_equation}
...
\label{og:sec:light_matter_interaction}
...
\begin{equation}
	H = \frac{1}{2m} [\vec{p} + q \vec{A}(\vec{r}, t)]^2 + V(\vec{r}) - q \phi(\vec{r}, t)
	\label{og:eq:hamiltonian}
\end{equation}

\noindent
where $\vec{p}$ is the momentum operator, $q$ the electric charge of the system, $V(\vec{r})$ the potential energy of the matter system, $\vec{A}(\vec{r}, t)$ the vector potential, and $\phi(\vec{r}, t)$ the scalar potential of the electromagnetic field \cite{cohen-tannoudjietal2008atomphotoninteractionsbasic}. Standard procedure is to go into the dipole approximation, meaning the spatial variation of the field over the extent of the molecule is negligible, and the Coulomb gauge, where $\nabla \cdot \vec{A} = 0$ and $\phi = 0$, to simplify the Hamiltonian to the commonly used form \todoref{find ref}
...
\begin{equation} \label{og:eq:dipole_approx_term}
	V(t) = - \hat{\vec{\mu}} \cdot \vec{\varepsilon}(t).
\end{equation}

\noindent
where $\vec{\varepsilon}(t)$ is the electric field and $\hat{\vec{\mu}}$ is the dipole operator of the matter system.
...
\begin{equation}
	\vec{P}(t) = \langle \hat{\vec{\mu}}(t) \rangle = \mathrm{Tr}[\hat{\vec{\mu}} \rho(t)].
	\label{og:eq:polarization_expectation_value}
\end{equation}
...
\begin{equation}
	\vec{P} = \varepsilon_0 (\chi^{(1)} \vec{\varepsilon} + \chi^{(2)} \vec{\varepsilon} \cdot \vec{\varepsilon} + \chi^{(3)} \vec{\varepsilon} \cdot \vec{\varepsilon} \cdot \vec{\varepsilon} + \ldots),
	\label{og:eq:nonlinear_polarization}
\end{equation}
...
\noindent
In the linear regime, the induced polarization $\vec{P}$ can be cut after the first contribution, which is directly proportional to the applied electric field $\vec{\varepsilon}$.
...
\begin{equation} \label{og:eq:electric_field_three_pulses_implemented}\vec{\varepsilon}(t)=\sum_{j=1}^3 \vec{E_0}^j \cdot g_j(t - t_j) \cdot e^{-i (\vec{k}_j \cdot \vec{r} - \omega_j t)} + \text{c.c.},
\end{equation}

\noindent where the normalized Gaussian envelope is defined as

\begin{equation} \label{og:eq:gaussian_envelope}
	g_j(u) = \frac{1}{\sqrt{2\pi} \sigma_j} \exp\left( -\frac{u^2}{2\sigma_j^2} \right),
\end{equation}

\noindent with $\sigma_j$ corresponding to the full width at half maximum (FWHM$_j$), and the factored form factoring out a common carrier $e^{-i \omega_L t}$ is

\begin{align} \label{og:eq:electric_field_factored}
	\vec{\varepsilon}(t) & = e^{-i \omega_L t} \sum_{j=1}^3 \vec{E}_j(t) + \text{c.c.} ,
\end{align}

\noindent
where we have defined the slowly varying envelopes

\begin{equation} \label{og:eq:slow_envelope_with_time_delay}
	\vec{E}_j(t) = \vec{E_0}^j \cdot g_j(t - t_j) \cdot e^{-i \omega_L t_j}.
\end{equation}

\noindent
Note that here the spatial, time-constant phase, was written as $\vec{k}_j \cdot \vec{r} = \omega_j t_j$ or, in the latter case, $\omega_L t_j$ \cite{greenetal2024vibrationalcoherenceshalfbroadband}.

\noindent
This is sometimes called the analytic–conjugate split $\vec{\varepsilon}^{(+)}(t)$, $\vec{\varepsilon}^{(-)}(t)=[\vec{\varepsilon}^{(+)}(t)]^*$. Then the third order component of the polarization Eq. \eqref{eq:nonlinear_polarization} is given by
...
\begin{equation}
	...
	\vec{E}_j^{(\sigma_1)}\vec{E}_k^{(\sigma_2)}\vec{E}_l^{(\sigma_3)}\,e^{-i\omega_S t},
\end{equation}
...
\begin{equation} \label{og:eq:general_third_order_freqs}
...
\label{og:sec:response_functions}
\todoidea{Note that the response function theory can be analytically done only for pure-dephasing baths!!}

In general, one can write Eq. \eqref{eq:polarization_expectation_value} in a perturbative series expansion

\begin{equation}
	\vec{P}(t) = \sum_n \vec{P}^{(n)}(t) = \mathrm{Tr}[\hat{\vec{\mu}} \rho^{(n)}(t)],
	\label{og:eq:polarization_expectation_value_perturbative}
\end{equation}
...
\noindent
The following equations are taken for the optical case of taking the dipole operator as the observable $A = \hat{\vec{\mu}}$, with the dipole approximation Hamiltonian (Eq. \eqref{eq:dipole_approx_term}), but generally they hold for other observable as well, just producing different expectation values. \todoref{<- find examples?}
The $n$-th order polarization can be written as the convolution of $n$ electric fields \cite{hamm2005principlesnonlinearoptical}
\begin{align}
	\vec{P}^{(n)}(t) & = \int_{0}^{\infty}\! dt_{n} \int_{0}^{\infty}\! dt_{n-1} \cdots \int_{0}^{\infty}\! dt_{1} \notag                                                             \\
	                 & \quad \vec{\varepsilon}\bigl(t-t_{n}\bigr)\, \vec{\varepsilon}\bigl(t-t_{n}-t_{n-1}\bigr)\,\cdots\, \vec{\varepsilon}\bigl(t-t_{n}-\cdots - t_{1}\bigr) \notag \\
	                 & \quad \vec{R}^{(n)}(t_{n},t_{n-1},\ldots,t_{1}),
	\label{og:eq:response_convolution}
\end{align}
\begin{equation}
	\vec{P}^{(n)}(t) = \int_0^\infty dt_1 \cdots dt_n \, \prod_{k=1}^n \vec{\varepsilon}\left(t - \sum_{j=k}^n t_j\right) \, \vec{R}^{(n)}(t_n, \ldots, t_1)
\end{equation}
\noindent
where the $n$-th response function is defined as

\begin{equation}
	\vec{R}^{(n)}(t_{n},\ldots,t_{1})
	\;=\;
	\left(-\frac{i}{\hbar}\right)^{n}
	\Big\langle
	\hat{\vec{\mu}}(t_{n}+\cdots + t_{1})\;[\hat{\vec{\mu}}(t_{n-1}+\cdots + t_{1}),\ldots [\hat{\vec{\mu}}(0),\rho(-\infty)]\ldots]
	\Big\rangle,
	\label{og:eq:response_function_R}
\end{equation}

\noindent
and describes the system's response to $n$ interactions with the electric field.
Here $\hat{\vec{\mu}}(t)$ is the dipole operator in the interaction picture.
Note, that the interactions at the different times $t - t_{n} - \ldots$ can occur in any order.
...
It is crucial that the first $n-1$ dipole operators are inside nested commutators, that represent how these field-interactions drive the system out of equilibrium. The last operator then represents the emitted field and is measured by being traced over the environmental degrees of freedom.
Explicitly, the commutators in Eq-\eqref{eq:response_function_R} contains $2^n$ terms with multiple actions of $\hat{\vec{\mu}}$ on the left / right on the density matrix.
Thought one can ignore the complex conjugate contributions, reducing to $2^{n-1}$ terms.
...
\noindent
Eq. \eqref{eq:response_convolution} is the central equation in two dimensional spectroscopy \cite{segarra-martietal2018accuratesimulationtwodimensional}. Each experimentally observed signal direction (phase matched condition Eq. %\eqref{eq:phase_matching}) 
isolates a subset of the possible Liouville-space pathways of the system.
Accordingly, the total response in that direction can be written as a sum over all contributing pathways,
\begin{equation}
	R^{(n)}(t) = \sum_{\text{path}} R_{\text{path}}(t),
	\label{og:eq:Liouville_pathways}
\end{equation}
...
\noindent
As mentioned in Sec \ref{subsec:third_order}, FWM is the method discussed in detail. It is a third-order process, so $n=3$ in Eq. \eqref{eq:response_convolution} and \eqref{eq:response_function_R}. The response function can be separated into different contributions, depending on the time ordering of the three field interactions. This leads to four main contributions, which can be represented with double-sided Feynman diagrams \cite{mukamel1995principlesnonlinearoptical}.


The setup can be seen in Fig. \ref{fig:fwm_box_car_setup}. Here three pulses with wavevectors $\vec{k}_1, \vec{k}_2, \vec{k}_3$ interact with the sample and generate a signal in the direction $\vec{k}_S$.

\noindent
This thesis will instead use a more microscopic non-perturbative (NP) approach.
Here first the total signal will be calculated, where all pulses are active, and then the desired order will be extracted by filtering out the linear contributions, where only the $n$-th pulse is active.
This way the third order signal can be calculated by

\begin{equation}
	\vec{P}^{(3)}(t) = \vec{P}_{\text{total}}(t) - \vec{P}_{(2)}(t) - \vec{P}_{(1)}(t) - \vec{P}_{(0)}(t),
	\label{og:eq:third_order_signal_numerical}
\end{equation}
...
\begin{figure}[ht]
	...
	\label{og:fig:fwm_box_car_setup}
\end{figure}
...
\begin{figure}[ht]
	...
	\label{og:fig:fwm_phase_cycling_setup}
\end{figure}
...
\todoidea{Also include how to simulate laser pulses:
	...
	$\vec{E_0}$ sets peak field (related to pulse energy via the mode area and duration).
...
\label{og:subsec:em_radiation_characterization}
...
\begin{equation}
	\vec{\varepsilon}(\vec{r},t) = \vec{E_0} \cos(\vec{k} \cdot \vec{r} - \omega t + \phi),
	\label{og:eq:plane_wave}
\end{equation}

\noindent
where $\vec{E_0}$ is the (real) amplitude, $\phi$ a phase-kick, $\vec{k}$ the wavevector (spatial angular frequency), and $\omega$ the angular frequency of the wave. The real field can be decomposed into complex positive and negative frequency parts as

\iffalse
	\begin{equation}
		\vec{\varepsilon}^{(+)} = \vec{E_0} e^{i(\vec{k} \cdot \vec{r} - \omega t + \phi)}
		\label{og:eq:positive_frequency_e_field}
	\end{equation}

	\noindent
	with $\vec{\varepsilon}^{(-)} = \vec{\varepsilon}^{(+)}^{*}$ and thus $\vec{\varepsilon} = 1/2 \{\vec{\varepsilon}^{(+)} + \vec{\varepsilon}^{(-)}\}$.
...
\label{og:subsec:rephasing_nonrephasing}
...
\label{og:subsec:phase_cycling_fourier_selection}
...
\begin{equation}
	\vec{P}^{\text{NL}}(t) = \sum_{l,m,n}
	e^{i(l \vec{k}_1 + m \vec{k}_2 + n \vec{k}_3)}
	\vec{P}_{l,m,n}(t) + c.c.,
	\label{og:eq:total_polarization_sum}
\end{equation}

\begin{equation}
	\vec{P}^{(3)}_{l,m,n}(t_{\text{coh}},T,t_{\text{det}}) \approx \sum_{l,m,n}
	e^{-i(l \phi_1 + m \phi_2 + n \phi_3)}
	\vec{P}^{(3)}(\phi_1,\phi_2,\phi_3;t_{\text{coh}},T,t_{\text{det}}),
	\label{og:eq:discrete_phase_cycling}
\end{equation}

\begin{align}
	\vec{P}^{(3)}_{l, m, n}(t_{\text{coh}},T,t_{\text{det}}) \approx \int_{0}^{2\pi}	\int_{0}^{2\pi} \int_{0}^{2\pi}
	 & d\phi_1 d\phi_2 d\phi_3
	e^{-i(l \phi_1 + m \phi_2 + n \phi_3)}                                  \\
	 & \vec{P}^{(3)}(\phi_1,\phi_2,\phi_3;t_{\text{coh}},T,t_{\text{det}}).
	\label{og:eq:continuous_phase_cycling}
\end{align}
...
\begin{align}
	\vec{k}_{\mathrm{R}}  & = -\vec{k}_1 + \vec{k}_2 + \vec{k}_3,
	                      & (l, m, n)                             & = (-1,+1,+1), \label{og:eq:rephasing_selection}    \\
	\vec{k}_{\mathrm{NR}} & = +\vec{k}_1 - \vec{k}_2 + \vec{k}_3,
	                      & (l, m, n)                             & = (+1,-1,+1), \label{og:eq:nonrephasing_selection}
\end{align}
...
\begin{equation}
	\vec{\varepsilon}_{k_S}(t_{\text{coh}},T,t_{\text{det}}) \propto i \vec{P}_{\vec{k}_S}(t_{\text{coh}},T,t_{\text{det}}).
	\label{og:eq:field_polarization_relation}
\end{equation}
...
After phase selection, the third order complex time-domain field $\vec{\varepsilon}_{k_S}(t_{\text{coh}}, T, t_{\text{det}})$ is Fourier transformed over the detection time $t_{\text{det}}$ and the coherence time $t_{\text{coh}}$ to construct the two-dimensional spectra \todoref{where is the eq?}%(cf. Eq.~\eqref{eq:2des_signal}):
...
\begin{align}
	...
	\vec{\varepsilon}_{R}(t_{\text{coh}}, T, t_{\text{det}}),
	\label{og:eq:rephasing_transform} \\
	S_{NR}(\omega_{\text{coh}}, T, \omega_{\text{det}})
	 & =
	\int dt_{\text{coh}} \int dt_{\text{det}} \;
	e^{- i \omega_{\text{coh}} t_{\text{coh}}} e^{- i \omega_{\text{det}} t_{\text{det}}}
	\vec{\varepsilon}_{NR}(t_{\text{coh}}, T, t_{\text{det}}).
	\label{og:eq:nonrephasing_transform}
\end{align}
...
\begin{equation}
	...
	\right\}.
	\label{og:eq:absorptive_spectrum}
\end{equation}
...
\label{og:sec:heterodyne_homodyne}
...
\begin{equation}
	I_{\text{HET}} \propto |\vec{\varepsilon}_{\text{S}} + \vec{\varepsilon}_{\text{LO}}|^2 \approx 2\Re{(\vec{\varepsilon}_{\text{LO}}^*\vec{\varepsilon}_{\text{S}})} + |\vec{\varepsilon}_{\text{LO}}|^2 + |\vec{\varepsilon}_{\text{S}}|^2,
	\label{og:eq:heterodyne}
\end{equation}

\noindent Since $|\varepsilon_{\text{LO}}| \gg |\varepsilon_{\text{S}}|$, the cross-term dominates, and the signal can be extracted by phase-cycling. In contrast to homodyne detection, the heterodyned signal is not background-free but sits on an offset $\int \varepsilon_{\text{LO}}(t)^2 \, dt$ which can be subtracted. Ideally, shaped pulses are used for the three input pulses and the local oscillator, yielding the response function directly: $\int \vec{\varepsilon}_{\text{LO}}(t) \vec{P}^{(3)}(t) \, dt = S^{(3)}(t_3, t_2, t_1)$, without convolution complications, where $t_1$, $t_2$, $t_3$ are the directly controlled delays. This is exactly the analytically defined third-order response function in Eq. \eqref{eq:response_function_R}, making experimental data and simulations directly interpretable. This is the main strength of heterodyne detection \cite{mukamel1995principlesnonlinearoptical}. Also, it provides both amplitude and phase information of the signal. The signal scales linearly with the third-order response \todoref{find ref} \todoidea{why is this good?}
...
\label{og:subsec:2d_spectroscopy}
...
\label{og:subsec:echo_applications}
...
\section{Third-Order Correlation Functions}
\label{og:sec:third_order_correlation_functions}
...
\begin{equation}
	\label{og:eq:third_order_correlation_function}
	R^{(3)}(t_3,t_2,t_1)
	= \left(\frac{i}{\hbar}\right)^3 \sum_{\alpha=1}^{4} \left[ R_{\alpha}(t_3,t_2,t_1) - R_{\alpha}^{*}(t_3,t_2,t_1) \right].
\end{equation}

\noindent
Each correlation function $R_{\alpha}$ encodes one double-sided Feynman diagram. The functions $R_1$ and $R_4$ describe nonrephasing pathways in which forward-evolving optical coherences persist during both coherence periods, whereas $R_2$ and $R_3$ capture rephasing pathways where the phase accumulated during $t_1$ is reversed during $t_3$. When the incident fields are written as $\vec{E}_j(t) = \vec{E}_j^{(+)}(t) + \vec{E}_j^{(-)}(t)$, the emitted field in direction $\vec{k}_S$ follows from $\vec{\varepsilon}^{(3)}_{\vec{k}_S} \propto \sum_{\alpha} R_{\alpha} \vec{E}_1^{\sigma_1} \vec{E}_2^{\sigma_2} \vec{E}_3^{\sigma_3}$ with the signs $\sigma_j = \pm$ determined by the bra/ket interaction history, reproducing the phase-matching rules in Eq.~\eqref{eq:four_wave_mixing}.
...
\begin{table}[ht]
	...
	\label{og:tab:third_order_correlation_functions}
	\begin{tabular}{llll}
		\hline

...
For spectroscopy, the system dipole operator is given by
\begin{equation}
	\hat{\vec{\mu}}_{\mathrm{S}}^{\,(N)} = \sum_{m,n} \mu_{(m,n)} \sigma_x^{(m,n)} \hat{e},
	\label{eq:Nqubit_dipole}
\end{equation}
where $\mu_{(m,n)}$ is the transition dipole moment for site $(m,n)$ and $\hat{e}$ is the unit vector along the polarization direction.
